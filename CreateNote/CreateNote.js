      ...
      if (this.data.useCamera == camera[1]) {
        ...
        ctx.takePhoto({
          ...
          success(res) {
            ...
            toShowNoteCargo.note.cameraFnChoice = "photo";
        ...
        if (lock) {
          ...
          ctx.startRecord({
            success(res) {
              ...
              setTimeout(function () {
                that.animation.backgroundColor("#EEE8AA").step();
                that.setData({
                  breathingEffection: that.animation.export()
                });
              }, 1000);
          ...
          ctx.stopRecord({
            success(res) {
              ...
              wx.vibrateLong();
              makeCameraNote = !makeCameraNote;
              toShowNoteCargo.note.cameraFnChoice = "video";              
    ...
    if (makeCameraNote) {
      wx.showActionSheet({
        ...
        success(res) {
          ...
          if (res.tapIndex == 0) {
            if (toShowNoteCargo.note.cameraFnChoice == "photo") {
              wx.previewImage({
                urls: [toShowNoteCargo.note.photoPath],
              });
              console.log("用户成功查看当前拍照记事，当前拍照记事路径为\n", toShowNoteCargo.note.photoPath);
            } else if (toShowNoteCargo.note.cameraFnChoice == "shoot") {
            ...
            makeCameraNote = false;
            toShowNoteCargo.note.cameraFnChoice = null;
            toShowNoteCargo.note.photoPath = null;
            toShowNoteCargo.note.videoPath = null;
